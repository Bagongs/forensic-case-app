...
export default function ProfileCorner({ active = false }) {
  ...
  const [showCloseModal, setShowCloseModal] = useState(false) // ⬅️ NEW
  ...
  return (
    <>
      <div ref={dropdownRef} className="relative">
        ...
        {open && (
          ...
          >
            ...
            {/* === SHOW CLOSE MODAL instead of logout === */}
            <button
      ...
      {/* === Close Confirmation Modal (GLOBAL) === */}
      <CloseConfirmationModal
        open={showCloseModal}
        onCancel={() => setShowCloseModal(false)}
        onConfirm={async () => {
          try {
            await logout() // ⬅️ HAPUS SESSION DULU
          } finally {
            window.system.quitApp() // ⬅️ BARU TUTUP APLIKASI
          }
        }}
      />
    </>
