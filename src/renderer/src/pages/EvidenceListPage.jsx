...
import { useCases } from '../store/cases' // ✅ ambil daftar case asli buat dropdown modal
...
export default function SuspectListPage() {
  ...
  // Ensure cases loaded for modal dropdown
  useEffect(() => {
  ...
  // ✅ Filter lokal MINIMAL:
  // - buang Unknown (UI lama kamu)
  // - kalau user pilih multi status, filter lokal karena BE belum dikirimi multi
  ...
  const totalSuspects = pagination?.total ?? displayRows.length
  const totalPages = Math.max(1, Math.ceil(totalSuspects / pageSize))
  const safePage = Math.min(page, totalPages)

  // ✅ case options diambil dari store cases (lebih lengkap)
  const caseOptions = useMemo(
    () => (cases ?? []).map((c) => ({ value: c.id, label: c.name })),
    [cases]
  )

  // ✅ AddPersonModal final sudah create sendiri,
  // jadi di sini cuma:
  // - close modal
  ...
  const handleSavePerson = async (payload) => {
    try {
      ...
      const newId = res?.data?.id ?? res?.data?.suspect_id ?? res?.data?.suspect?.id ?? res?.id

      setModal(false)

      await fetchSuspects({ skip: 0, limit: pageSize })
      await fetchSuspectSummary().catch(() => {})

      if (newId) nav(`/suspects/${newId}`)
    } catch (err) {
      console.error('[SuspectListPage] after create failed:', err)
      setModal(false)
    }
  }
